...
if (count($_POST) > 0) {
  ...
  $t = '&token=' . $zbp->GetToken('AppCentre');
	$zbp->SetHint('good', '提交成功！<a href="submit.php?type=plugin&id=' . $app->id . $t . '">现在立刻上传到应用中心！</a>');
...
<?php } ?>
    ...
    <tr>
      <td><p><b>· 【高级】依赖插件列表（以|分隔）</b>(可选)</p></td>
      <td><p>&nbsp;
    ...
    <tr>
      <td><p><b>· 【高级】冲突插件列表（以|分隔）</b>(可选)</p></td>
      <td><p>&nbsp;
...
<?php if(property_exists($app,'advanced_existsfunctions')){ ?>
    <tr>
      <td><p><b>· 【高级】插件依赖函数列表（以|分隔）</b>(可选)</p></td>
      <td><p>&nbsp;
