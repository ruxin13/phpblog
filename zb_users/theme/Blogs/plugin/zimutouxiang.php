<?php
function Blogs_tt_touxiang_generate_first_letter_uri($name){
	global $zbp;
$name2=Blogs_tt_touxiang_Getzimu($name);//新加
...
$name=$name2;
			...
			if (extension_loaded('mbstring')){ // check if mbstring is loaded to allow multibyte string operations
				...
				$file_name_mb=Blogs_tt_touxiang_Getzimu($file_name_mb);//新加
		...
		$avatar_uri ="{$zbp->host}zb_users/theme/{$zbp->theme}/image/48/{$file_name}.png";
...
function Blogs_tt_touxiang_Getzimu($str) 
{ 
