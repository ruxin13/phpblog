...
function Blogs_pytags_show() {
    global $zbp;
    $tags = $zbp->GetTagList('*', '', array(
        'tag_Count' => 'DESC'
    ) , null, null);
    $list = array();
    $content = '';
    foreach ($tags as $tag) {
        for ($i = 65; $i <= 90; $i++) {
            if (strtoupper(Blogs_Chinese_to_PY::getPY($tag->Name, 'one')) == chr($i)) {
                $list[chr($i) ][$tag->Url] = array(
                    $tag->Name,
                    $tag->Count
                );
            }
    ...
    ksort($list);
    $content.= "<ul id='tag_letter'>";
    for ($i = 65; $i <= 90; $i++) {
        if (isset($list[chr($i) ])) {
            $st = $list[chr($i) ];
            if (is_array($st)) {
                $content.= "<li class=\"col-sm-1 col-xs-2\"><a href='#" . chr($i) . "'>" . chr($i) . "</a></li>";
            }
            continue;
        }
        $content.= "<li class=\"col-sm-1 col-xs-2\">" . chr($i) . "</li>";
    }
    for ($i = 48; $i <= 57; $i++) {
        if (isset($list[chr($i) ])) {
            $st = $list[chr($i) ];
            if (is_array($st)) {
                $content.= "<li><a href='#" . chr($i) . "'>" . chr($i) . "</a></li>";
            }
            continue;
        }
        $content.= "<li>" . chr($i) . "</li>";
    ...
    $content.= "</ul>";
    $content.= "<ul id='all_tags'>";
    for ($i = 65; $i <= 90; $i++) {
        if (isset($list[chr($i) ])) {
            $st = $list[chr($i) ];
            if (is_array($st)) {
                $content.= "<li id='" . chr($i) . "'><h4 class='tag_name'>" . chr($i) . "</h4>";
                foreach ($st as $tagurl => $taginfo) {
                    $content.= "<a href='" . $tagurl . "'>" . $taginfo[0] . "<sup>(" . $taginfo[1] . ")</sup></a>";
                }
            }
        }
    }
...
function Blogs_cx_archives_list() {
		global $zbp;
