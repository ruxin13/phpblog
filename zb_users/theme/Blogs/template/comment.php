...
{/php}
	<span class="reply_tz">{$comment.Time()}<a rel="nofollow" class="comment-reply-link" href="#comment" onclick="zbp.comment.reply('{$comment.ID}')" aria-label="回复给{$comment.Author.StaticName}">&nbsp;<i class="fa fa-reply"></i>&nbsp;回复</a></span><br> </div> <p><a class="at" href="#comment-{$comment.ID}">@{$atname}</a>{php}echo Blogs_biaoqing($comment->Content){/php}</p>
{else}
{php}
