...
<article id="post-{$article.ID}" class="archive-list">
		...
		<div class="entry-content">
			...
			<span class="entry-meta">
				<span class="post_cat">
				<a href="{$article.Category.Url}" target="_blank" rel="category tag">{$article.Category.Name}</a>			</span>
				<span class="post_spliter">•</span>
				<span class="date" title="{$article.Time()}">{php}echo zbpNana_timeago($article->Time());{/php}</span>			
			</span>		
			
			<div class="archive-content">
			{if strlen($article.Intro)>0}
				{php}$intro= trim(SubStrUTF8(TransferHTML($article->Intro,'[nohtml]'),80)).'...';{/php}
				{else}
